...
class WriteData():
    ...
    def write_id_title_map(self):
        ...
        with open('../data/index/id_title_map.txt', 'a') as f:  # append mode
            f.write('\n'.join(temp_id_title))
            f.write('\n')
    ...
    def write_intermed_index(self):
        ...
        for word, posting in tqdm(temp_index_map):
            temp_index.append(word + '-' + posting)  # "word-doc_id:t`tnum`b`bnum`...doc_id...doc_id..."

        with open(f'../data/index/index_{num_files}.txt', 'w') as f:
            f.write('\n'.join(temp_index))
    ...
    def write_final_files(self, data_to_merge, num_files_final):
        ...
        title_dict, body_dict, category_dict, infobox_dict, link_dict, reference_dict = \
        ...
        final_titles, final_body_texts, final_categories, \
        final_infoboxes, final_links, final_references = [], [], [], [], [], []
        ...
        with open('../data/index/tokens_info.txt', 'a') as f:
    ...
    def write_diff_postings(self, tag_type, final_tag, num_files_final):
        ...
        with open(f'../data/index/{tag_type}_data_{str(num_files_final)}.txt', 'w') as f:
            f.write('\n'.join(final_tag))
...
class CreateIndex():
    ...
    def index(self, title, body, category, infobox, link, reference):
        ...
        # index a single page
        words_set, title_dict, body_dict, category_dict, infobox_dict, link_dict, reference_dict = \
            set(), defaultdict(int), defaultdict(int), defaultdict(int), defaultdict(int), \
        ...
        for word in words_set:
            ...
            if not is_rep:
                ...
                index_map[word] += posting  # posting list, like "word -> doc_id:t`num`b`num`...doc_id:...doc_id:..."
...
class MergeFiles():
    ...
    def merge_files(self):
        ...
        while i < self.num_itermed_files:
            ...
            files_data[i] = open(f'../data/index/index_{i}.txt', 'r')
                ...
                if is_file_empty[i] == 0:
                    ...
                    if token == postings[i][0]:
                        ...
                        if len(line[i]):
                            ...
                            os.remove(f'../data/index/index_{str(i)}.txt')
...
if __name__ == '__main__':
    ...
    with open('../data/index/num_pages.txt', 'w') as f:
        f.write(str(num_pages))

    num_tokens_final = 0
    with open('../data/index/tokens_info.txt', 'r') as f:
        for line in f:
            num_tokens_final += 1
    ...
    with open(f'../data/index/tokens_info.txt', 'r') as f:
        for line in tqdm(f):
            if line[0] in char_list:
                with open(f'../data/index/tokens_info_{line[0]}.txt', 'a') as t:
                    t.write(line.strip())
                    t.write('\n')
                ...
                with open(f'../data/index/tokens_info_others.txt', 'a') as t:
                    t.write(line.strip())
                    t.write('\n')

    for ch in tqdm(char_list):
        tok_count = 0
        with open(f'../data/index/tokens_info_{ch}.txt', 'r') as f:
            for line in f:
                tok_count += 1

        with open(f'../data/index/tokens_info_{ch}_count.txt', 'w') as f:
            f.write(str(tok_count))

    for num in tqdm(num_list):
        tok_count = 0
        with open(f'../data/index/tokens_info_{num}.txt', 'r') as f:
            for line in f:
                tok_count += 1

        with open(f'../data/index/tokens_info_{num}_count.txt', 'w') as f:
            f.write(str(tok_count))

    try:
        tok_count = 0
        with open('../data/index/tokens_info_others.txt', 'r') as f:
            tok_count += 1

        with open(f'../data/index/tokens_info_others_count.txt', 'w') as f:
            f.write(str(tok_count))
    except:
        pass

    os.remove('../data/index/tokens_info.txt')
